...
const renderTable = (dado) => {
  let injetar = "";

  dado.forEach((inserir) => {
    injetar += `
    <tr>
      <td>${new Date(inserir.date).toLocaleDateString("pt-BR")}</td>
      <td>${inserir.customer.first_name} ${inserir.customer.last_name}</td>
      <td>${inserir.customer.phone}</td>
      <td style='padding-left: 30px'>${inserir.store.name}</td>
      <td>${inserir.store.phone}</td>
      <td>${inserir.type === "OUT" ? "SAÍDA" : "ENTRADA"}</td>
      <td class ='text-center'>${inserir.amount}</td>
      <td>${inserir.price.toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
      })}</td>
      <td>${(inserir.price * inserir.amount).toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
      })}</td>
    </tr>
  ...
  document.getElementById("tabela-principal").innerHTML = injetar;
};

const renderSelect = () => {
  let injet = `<option value=''>TIPO</option>`;

  const arr = dados.map((percorrer) => percorrer.type);
  const typesEncontrados = new Set(arr);

  typesEncontrados.forEach((inserir) => {
    injet += `<option value='${inserir}'>${
      inserir === "OUT" ? "SAÍDA" : "ENTRADA"
    }</option>`;
  });
  document.getElementById("select").innerHTML = injet;
};

const filterSelect = () => {
  const idSelect = document.getElementById("select").value;
  console.log(idSelect);

  const selectFiltrado = dados.filter((preenchido) => {
    let valido = true;
    if (idSelect && idSelect !== preenchido.type) {
      valido = false;
    }
    return valido;
  });
  renderTable(selectFiltrado);
  resultadoType(selectFiltrado);
};

const resultadoType = (caixa) => {
  let entrada = 0;
  let saida = 0;

  caixa.forEach((dados) => {
    if (dados.type === "IN") {
      entrada = entrada + dados.price * dados.amount;
    }

    if (dados.type === "OUT") {
      saida = saida - dados.price * dados.amount;
    }
  });
  ...
  html += ` 
  <div style="grid-column: 2;justify-self: right; border: 2px solid green; 
  border-radius: 5px; width: 100%">
  <label class="resultado" 
  style="  
  padding: 5px "}>Total de entradas:${entrada.toLocaleString("pt-br", {
    style: "currency",
    currency: "BRL",
  })}</label>
  </div>

  <div style="grid-column: 3;justify-self: right; border: 2px solid red; 
  border-radius: 5px; width: 100% ">
  
  <label class="resultado" 
  style=" 
  padding: 5px "> Total de saídas:${saida.toLocaleString("pt-br", {
    style: "currency",
    currency: "BRL",
  })} </label>
  </div>

  <div style="grid-column: 4;justify-self: right; border: 2px solid blue; 
  border-radius: 5px; width: 100%">
   
  <label class="resultado" 
   style="  
  padding: 5px "> Saldo:${(entrada + saida).toLocaleString("pt-br", {
    style: "currency",
    currency: "BRL",
  })} </label>
  </div>`;
